...
bool validNumber(const char * arg);
const UnaryOperator validUnaryOperator(const char * arg, const RoyStack * stack);
const BinaryOperator validBinaryOperator(const char * arg, const RoyStack * stack);
void doNumber(const char * arg, RoyStack * stack);
void doUnaryOperator(const UnaryOperator op, RoyStack * stack);
void doBinaryOperator(const BinaryOperator op, RoyStack * stack);
void doVariable(const char * arg);
void doError(void);
...
void rpc(RoyShell * shell) {
  ...
  for (int i = 1; i != roy_shell_argument_count(shell); i++) {
    ...
    if (unyOp = validUnaryOperator(arg, stack)) {
      doUnaryOperator(unyOp, stack);
    } else
    if (binOp = validBinaryOperator(arg, stack)) {
      doBinaryOperator(binOp, stack);
    }
